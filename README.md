...
epub-tools cover BOOK.epub -o cover.png
...
- **`nav`** reads the table of contents from the EPUB 3 nav document or the
  EPUB 2 NCX.
- **`cover`** locates the cover image via the EPUB 3 `properties="cover-image"`
  manifest entry, falling back to the EPUB 2 `<meta name="cover">` reference, and
  writes its bytes out unchanged.
